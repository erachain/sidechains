...
Для того чтобы ноды (узлы, пиры) увидели друг друга, необходимо иметь хотя бы одну ноду с возможностью подключения к ней других нод. Она должна иметь статический IP и на ней нужно открыть доступ для внешних вызовов, см. ниже. Так же ее IP нужно указать в списке начальных нод вашей сети в файле `peers-side.json`. Чем больше таких нод, тем надежней сеть. Пример синтаксиса внутри файла можно посмотреть в других подобных файлах (например `peers.json`). Заполните `peers-side.json` и раздайте его на другие ноды вашей сети.  
Для того чтобы нода принимала внешние вызовы, на вычислительной машине (или "виртуальной машине"), на которой она запущена, необходимо открыть порт сети **9056** для внешних вызовов. Для справки - порт 9057 используется для доступа к API и blockexplorer. А порт 9058 дает доступ к RPC. Это делается в файреволе операционной системы вычислительной машины. Обычно достаточно прописать входящее правило для этих портов в FireWall операционной системы машины, но иногда необходим прописать еще доступ к виртуальной машине в личном кабинете провайдера виртуальных машин, где вы арендуете свою виртуальную машину. Вдобавок если ваша машина находится за NAT, нужно пробросить порты в вашей локальной сети.
...
Если Вы поменяли протокол на уже созданной цепочке, то необходимо удостовериться что нет конфликтов со старой версией протокола. На одной ноде где обновлен файл `sidePROTOCOL.json`удалите базу цепочки и заново пересинхронизируйте все данные цепочки. Если нода не блокирует другие ноды со старым протоколом, то смело копируйте новый протокол на другие ноды и там так же нужно сделать полную пересинхронизацию цепочек. Лучше делать это не у всех нод сети разом, а разбить на несколько шагов чтобы генерация блоков в сети не останавливалась - обновляйте за раз примерно 1/4 часть нод.
Если же нода с новым протоколом начала банить другие ноды, то включите в протоколе (задайте параметр) не проверять блоки до заданной высоты цепочки.
