...
> Genesis block (генесиз блок) - это первый блок цепочки данных, в котором происходит первоначальное распределение пакетов для форжинга (ERA) по счетам, которые находятся в кошельках узлов (нод), которые должны быть включены и будут собирать блоки цепочки
